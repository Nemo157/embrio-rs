...
impl embrio::timer::Timer for Timer<TIMER1> {
    type Error = !;

    type Timeout = Timeout<TIMER1>;

    type Interval = Interval<TIMER1>;
...
impl Future for Timeout<TIMER1> {
    type Item = Timer<TIMER1>;
    type Error = !;
...
impl Stream for Interval<TIMER1> {
    type Item = ();
    type Error = !;
