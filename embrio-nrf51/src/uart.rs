...
use gpio::{Pin, mode::{Input, Output, PushPull, Floating}};
use zst_ref::ZstRef;
...
impl<'a> Uart<'a> {
    pub fn new(
        ...
        uart.pseltxd.write(|w| unsafe { w.bits(txpin.get_id() as u32) });
        uart.pselrxd.write(|w| unsafe { w.bits(rxpin.get_id() as u32) });
    ...
    pub fn split(self) -> (Tx<'a>, Rx<'a>) {
        let Uart { uart, _txpin, _rxpin } = self;
...
impl<'a> embrio::io::Read for Rx<'a> {
    ...
    fn poll_read(self: mem::Pin<Self>, _cx: &mut task::Context, buf: &mut [u8]) -> Poll<usize, Self::Error> {
        if buf.len() == 0 {
...
impl<'a> embrio::io::Write for Tx<'a> {
    ...
    fn poll_write(self: mem::Pin<Self>, _cx: &mut task::Context, buf: &[u8]) -> Poll<usize, Self::Error> {
        if buf.len() == 0 {
            return Ok(Async::Ready(0));
        }
    ...
    fn poll_flush(self: mem::Pin<Self>, _cx: &mut task::Context) -> Poll<(), Self::Error> {
        self.uart.intenclr.write(|w| w.txdrdy().clear());
        if self.uart.events_txdrdy.read().bits() == 1 {
            Ok(Async::Ready(()))
        } else {
    ...
    fn poll_close(self: mem::Pin<Self>, _cx: &mut task::Context) -> Poll<(), Self::Error> {
