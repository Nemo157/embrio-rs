...
pub fn read_exact<'a, 'b: 'a, R: Read + 'a>(mut this: Pin<'a, R>, buf: &'b mut [u8])
    -> impl StableFuture<Item=(), Error=Error<R::Error>> + Captures<'a> + Captures<'b>
{
    async_block_pinned! {
        let mut position = 0;
        while position < buf.len() {
            let amount = await_poll!(|cx| Pin::borrow(&mut this).poll_read(cx, &mut buf[position..]))?;
            position += amount;
            if amount == 0 {
...
pub fn write_all<'a, 'b: 'a, W: Write + 'a>(mut this: Pin<'a, W>, buf: &'b [u8])
    -> impl StableFuture<Item=(), Error=Error<W::Error>> + Captures<'a> + Captures<'b>
{
    async_block_pinned! {
        let mut position = 0;
        while position < buf.len() {
            let amount = await_poll!(|cx| Pin::borrow(&mut this).poll_write(cx, &buf[position..]))?;
            position += amount;
            if amount == 0 {
                Err(Error::WriteZero)?;
            }
...
pub fn flush<'a, W: Write + 'a>(mut this: Pin<'a, W>)
...
pub fn close<'a, W: Write + 'a>(mut this: Pin<'a, W>)
    -> impl StableFuture<Item=(), Error=W::Error> + Captures<'a>
{
    async_block_pinned! {
